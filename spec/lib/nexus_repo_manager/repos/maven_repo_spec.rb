...
RSpec.describe 'NexusRepoManager::MavenRepo' do
  let(:data) { {
    'online' => 'online',
    'storage' => 'storage',
    'cleanup' => 'cleanup',
  ...
  before(:each) do
    @repo = NexusRepoManager::MavenRepo.new(data)
  end
  ...
  describe '#generate_options' do
    ...
    it 'supports maven settings' do
      expect(@repo.generate_options['maven']).to eq(data['maven'])
    end
  end
end
